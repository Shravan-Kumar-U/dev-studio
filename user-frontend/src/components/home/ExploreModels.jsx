...
const ExploreModels = () => {
  ...
  useEffect(() => {
    ...
    const scrollStep = (currentTime) => {
      ...
      if (!isPaused && !isDragging.current) {
        ...
        scrollContainer.scrollLeft += (AUTO_SCROLL_SPEED * deltaTime) / 1000;
  ...
  return (
    <div
      id="models"
      className="bg-[var(--bg-color)] py-12 sm:py-16 lg:py-24 border-b border-[var(--border-color)] overflow-hidden"
    >
      <div className="w-full px-4 sm:px-6 lg:px-16 2xl:px-32 mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 sm:mb-12 gap-4 sm:gap-6 relative z-10">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-extrabold text-[var(--text-color)] mb-2 sm:mb-3">
              Explore 3D Models
            </h2>
            <p className="text-[var(--text-muted)] text-sm sm:text-lg">
              Find the perfect model for your next 3D print.
            </p>
          </div>

          {/* --- HIGH-ATTENTION ANIMATED BUTTON WRAPPER --- */}
          <div className="relative group inline-flex self-start md:self-auto shrink-0 mt-2 md:mt-0">
            {/* Outer Amplified Aura - Kept intact for the breathing effect */}
            <div className="absolute -inset-1 bg-[var(--color-primary)] rounded-full blur-md opacity-30 animate-pulse group-hover:opacity-60 transition-opacity duration-500"></div>

            {/* Inner Sharper Glow - Kept intact */}
            <div className="absolute inset-0 bg-[var(--color-primary)] rounded-full blur-sm opacity-40 animate-pulse delay-75"></div>

            {/* The Actual Link Button (Slimmed down) */}
            <Link
              to="/models"
              className="relative flex items-center gap-2 px-4 py-2.5 sm:px-6 sm:py-3 bg-[var(--surface-color)] text-[var(--text-color)] text-xs sm:text-sm font-bold uppercase tracking-wide border border-[var(--color-primary)] rounded-full hover:bg-[var(--color-primary)] hover:text-slate-900 transition-all duration-300 hover:shadow-[0_0_20px_var(--color-primary)] hover:-translate-y-1"
            >
              <LayoutGrid
                size={16}
                className="text-[var(--color-primary)] group-hover:text-slate-900 transition-colors sm:w-5 sm:h-5"
              />
              <span className="mt-0.5">View All Products</span>
              <ArrowRight
                size={14}
                className="transform group-hover:translate-x-1.5 transition-transform sm:w-[18px] sm:h-[18px]"
              />
            </Link>
          </div>
        </div>
      ...
      {loading ? (
            ...
            {products.map((product, index) => (
              ...
              >
                ...
                <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-20 bg-black/70 backdrop-blur-md text-white text-xs sm:text-sm font-bold px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border border-white/10 shadow-lg flex items-center gap-1 sm:gap-1.5 opacity-90 sm:group-hover:opacity-100 group-active:opacity-100 sm:group-hover:scale-105 group-active:scale-105 transition-all duration-300">
                  <Tag
                    size={12}
                    className="text-[var(--color-primary)] sm:w-3.5 sm:h-3.5"
                  />
        ...
        <div className="text-center py-8 sm:py-12 text-sm sm:text-base text-[var(--text-muted)]">
